# 贡献指南

> 🔧 Mock Server 开发贡献完全指南
>
> 欢迎参与项目开发，一起打造更好的 Mock Server！

## 📚 开发导航

| 文档 | 用途 | 读者 |
|------|------|------|
| [📖 项目介绍](README.md) | 项目概述和快速开始 | 所有开发者 |
| [📊 技术总览](PROJECT_SUMMARY.md) | 架构和技术栈详解 | 架构师、开发者 |
| [🚀 部署指南](DEPLOYMENT.md) | 环境搭建和部署 | DevOps、开发者 |
| [🧪 测试框架](tests/README.md) | 测试指南和执行 | QA、开发者 |

## 行为准则

参与本项目即表示您同意遵守我们的行为准则。请保持友善、尊重和专业。

## 如何贡献

### 报告 Bug

如果您发现 Bug，请通过 GitHub Issues 报告：

1. 使用描述性的标题
2. 提供详细的复现步骤
3. 说明预期行为和实际行为
4. 附上环境信息（Go 版本、操作系统等）
5. 如有可能，提供最小化的复现示例

**Bug 报告模板：**

```markdown
### 问题描述
简要描述问题

### 复现步骤
1. 第一步
2. 第二步
3. ...

### 预期行为
应该发生什么

### 实际行为
实际发生了什么

### 环境信息
- Go 版本：
- 操作系统：
- Mock Server 版本：
- MongoDB 版本：

### 附加信息
日志、截图或其他相关信息
```

### 提出新功能

在提出新功能建议前，请先查看：

1. 是否已有类似的 Issue 或 PR
2. 功能是否符合项目定位
3. 是否愿意自己实现该功能

**功能请求模板：**

```markdown
### 功能描述
清晰描述您想要的功能

### 使用场景
为什么需要这个功能？解决什么问题？

### 建议的实现方案
（可选）您认为如何实现这个功能

### 替代方案
（可选）是否考虑过其他方案
```

### 提交代码

1. Fork 本仓库
2. 创建功能分支（`git checkout -b feature/amazing-feature`）
3. 编写代码并添加测试
4. 确保所有测试通过
5. 提交更改（`git commit -m 'Add some amazing feature'`）
6. 推送到分支（`git push origin feature/amazing-feature`）
7. 创建 Pull Request

## 开发流程

### 准备开发环境

1. **安装依赖**

```bash
# 克隆仓库
git clone https://github.com/gomockserver/mockserver.git
cd mockserver

# 安装 Go 依赖
go mod download

# 启动 MongoDB
docker run -d -p 27017:27017 --name mongodb mongo:6.0
```

2. **运行项目**

```bash
# 编译
make build

# 运行
make run

# 或直接运行
go run cmd/mockserver/main.go
```

3. **运行测试**

```bash
# 运行所有测试
make test

# 运行单元测试
make test-unit

# 运行集成测试
make test-integration

# 查看覆盖率
make coverage
```

### 项目结构

```
gomockserver/
├── cmd/                    # 可执行程序入口
│   └── mockserver/        # 主程序
├── internal/              # 内部代码（不对外暴露）
│   ├── adapter/          # 协议适配器
│   ├── api/              # API 处理器
│   ├── config/           # 配置管理
│   ├── engine/           # 规则匹配引擎
│   ├── executor/         # Mock 执行器
│   ├── models/           # 数据模型
│   ├── repository/       # 数据访问层
│   └── service/          # 服务层
├── pkg/                   # 公共库（可对外暴露）
│   ├── logger/           # 日志工具
│   └── utils/            # 通用工具
├── tests/                 # 测试相关
│   ├── integration/      # 集成测试
│   ├── performance/      # 性能测试
│   └── smoke/            # 冒烟测试
├── docs/                  # 文档
├── tests/                 # 测试和脚本工具
└── docker/                # Docker 相关
```

## 代码规范

### Go 代码规范

遵循 [Effective Go](https://golang.org/doc/effective_go.html) 和 [Go Code Review Comments](https://github.com/golang/go/wiki/CodeReviewComments)。

**基本规则：**

1. **格式化**：使用 `gofmt` 或 `goimports`
2. **命名**：
   - 包名：小写，简短，不使用下划线或驼峰
   - 导出标识符：使用大驼峰（PascalCase）
   - 非导出标识符：使用小驼峰（camelCase）
   - 常量：根据作用域使用大驼峰或小驼峰
3. **注释**：
   - 所有导出的函数、类型都应有注释
   - 注释以名称开头
   - 使用完整的句子
4. **错误处理**：
   - 不要忽略错误
   - 错误信息使用小写，不以标点结尾
   - 使用 `fmt.Errorf` 包装错误
5. **并发**：注意 goroutine 泄漏和数据竞争

**代码示例：**

```go
// GoodExample 这是一个好的示例函数
// 它展示了正确的命名、注释和错误处理
func GoodExample(input string) (string, error) {
    if input == "" {
        return "", fmt.Errorf("input cannot be empty")
    }
    
    result, err := processInput(input)
    if err != nil {
        return "", fmt.Errorf("failed to process input: %w", err)
    }
    
    return result, nil
}
```

### 代码质量检查

在提交前运行：

```bash
# 代码格式化
make fmt

# 代码检查
make lint

# 运行测试
make test
```

## 测试要求

### 单元测试

- 所有新功能必须包含单元测试
- 核心模块的覆盖率应 > 80%
- 测试文件命名：`*_test.go`
- 使用 `testify` 进行断言

**示例：**

```go
func TestMatchEngine_Match(t *testing.T) {
    tests := []struct {
        name    string
        rule    *models.Rule
        request *adapter.Request
        want    bool
    }{
        {
            name: "精确路径匹配",
            rule: &models.Rule{
                MatchCondition: models.MatchCondition{
                    Method: "GET",
                    Path:   "/api/users",
                },
            },
            request: &adapter.Request{
                Method: "GET",
                Path:   "/api/users",
            },
            want: true,
        },
        // 更多测试用例...
    }
    
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            // 测试实现
            got := match(tt.rule, tt.request)
            assert.Equal(t, tt.want, got)
        })
    }
}
```

### 集成测试

- 测试完整的业务流程
- 使用 Docker 测试环境
- 测试文件放在 `tests/integration/`

### 性能测试

- 关键路径需要性能测试
- 使用 Go benchmark 或 wrk
- 文档化性能基准

## 提交规范

### Commit Message 格式

使用 [Conventional Commits](https://www.conventionalcommits.org/) 规范：

```
<type>(<scope>): <subject>

<body>

<footer>
```

**Type 类型：**

- `feat`: 新功能
- `fix`: Bug 修复
- `docs`: 文档更新
- `style`: 代码格式（不影响代码运行）
- `refactor`: 重构（既不是新功能也不是 Bug 修复）
- `perf`: 性能优化
- `test`: 测试相关
- `chore`: 构建过程或辅助工具的变动

**示例：**

```bash
feat(engine): 添加正则表达式匹配支持

实现了基于正则表达式的路径匹配功能，支持更灵活的规则配置。

Closes #123
```

### Pull Request

**PR 标题格式：**

```
[Type] Brief description
```

**PR 描述模板：**

```markdown
## 变更说明
简要描述本 PR 的变更内容

## 变更类型
- [ ] 新功能
- [ ] Bug 修复
- [ ] 文档更新
- [ ] 性能优化
- [ ] 重构
- [ ] 测试
- [ ] 其他

## 相关 Issue
Closes #issue_number

## 测试
说明如何测试这些变更

## 检查清单
- [ ] 代码遵循项目规范
- [ ] 已添加必要的测试
- [ ] 所有测试通过
- [ ] 已更新相关文档
- [ ] Commit 消息符合规范
```

### Code Review

**审查重点：**

1. 代码质量和可读性
2. 测试覆盖率
3. 性能影响
4. 向后兼容性
5. 文档完整性

**反馈指南：**

- 提出建设性意见
- 说明理由
- 提供具体建议
- 保持友好和尊重

## 问题反馈

### 安全问题

如果发现安全漏洞，请**不要**公开报告。请发送邮件至：

**security@gomockserver.com**

我们会尽快响应并处理。

### 一般问题

- GitHub Issues：功能请求、Bug 报告
- GitHub Discussions：一般讨论、问题咨询
- Email：support@gomockserver.com

## 开发工具推荐

### IDE 配置

**VS Code**

推荐插件：
- Go (golang.go)
- Go Test Explorer
- GitLens
- MongoDB for VS Code

**GoLand**

- 内置完整的 Go 支持
- 集成测试运行器

### 常用命令

```bash
# 构建
make build

# 运行
make run

# 测试
make test          # 所有测试
make test-unit     # 单元测试
make test-int      # 集成测试

# 代码质量
make fmt           # 格式化
make lint          # 代码检查
make vet           # Go vet

# 覆盖率
make coverage      # 生成覆盖率报告

# Docker
make docker-build  # 构建镜像
make docker-run    # 运行容器
make docker-test   # Docker 测试环境
```

## 发布流程

1. 更新版本号（遵循语义化版本）
2. 更新 CHANGELOG.md
3. 创建 Release PR
4. 合并后打标签
5. GitHub Actions 自动发布

## 获得帮助

如果您在贡献过程中遇到问题：

1. 查看现有文档和 Issues
2. 在 GitHub Discussions 提问
3. 联系维护者

## 致谢

感谢所有贡献者的付出！您的每一个贡献都让这个项目变得更好。

---

**Happy Coding! 🚀**
