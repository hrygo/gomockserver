...
本版本主要实现 WebSocket 协议支持和脚本匹配功能，增强 Mock Server 的协议覆盖和匹配能力。

#### ✨ Added (Core Features)

**WebSocket 协议支持** (`internal/adapter/websocket_adapter.go`)
- WebSocket 适配器实现，完整支持 WebSocket 协议
- 连接管理：
  - 支持多个并发连接
  - 可配置最大连接数（默认 1000）
  - 连接添加/移除/查询功能
...
**脚本匹配引擎** (`internal/engine/script_engine.go`)
- 基于 goja JavaScript 引擎实现
- 安全沙箱：
  - 禁用危险函数（require、eval、Function）
